...
const bindMessageHandler = (webSocketInstance: WebSocketLike, url: string) => {
...
const bindOpenHandler = (webSocketInstance: WebSocketLike, url: string) => {
...
const bindCloseHandler = (webSocketInstance: WebSocketLike, url: string) => {
...
const bindErrorHandler = (webSocketInstance: WebSocketLike, url: string) => {
  webSocketInstance.onerror = (error: WebSocketEventMap['error']) => {
    getSubscribers(url).forEach((subscriber) => {
      ...
      if (isEventSourceSupported && webSocketInstance instanceof EventSource) {
        // eslint-disable-next-line @typescript-eslint/no-unused-expressions
        subscriber.optionsRef.current.onClose &&
          subscriber.optionsRef.current.onClose({
            ...error,
            code: 1006,
