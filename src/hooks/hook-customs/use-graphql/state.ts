...
export class GraphQLStateManager<TResultData, TError extends ErrorType> {
  ...
  private async submitAsync(variables?: Record<string, any>) {
    ...
    const globalConfig =
      this.globalConfig ?? (defaultGraphQLConfig as GraphQLConfig<TResultData, TError, unknown>);
    const config =
      this.config ?? (defaultGraphQLConfig as GraphQLConfig<TResultData, TError, unknown>);
