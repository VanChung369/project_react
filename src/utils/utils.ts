...
function findIgnore(character: OpenCloseCharacter, texts: string[], index: number) {
  if (!character.ignore) {
    return null;
  }
...
export function splitText(text: string, splitOptions: string | SplitOptions): string[] {
  const {
    separator = ',',
    isSeparateFirst,
